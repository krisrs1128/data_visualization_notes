...
1. Vector data are used to store geometric spatial data. Specifically, there are
...
5. To visualize vector data, we can use the tmap package. Like ggplot2, this
...
6. Often, we want to layer on several vector objects. `tmap` makes this possible
...
1. It's natural to use maps to select regions of interest. We may also need
extra pan and zoom functionality to make the most of very high-resolution
spatial data.

1. Mosaic vgplot is especially well-suited to selection interactions. We need
...
1. Let's build an example that allows us to make a choropleth map where counties
...
1. Next, let's link this map with a histogram of the unemployment rates. This
...
1. The machinery for working with raster data in Mosaic is not as mature as that
...
1. The visual encoding channels are all relatively straightforward. One subtlety
...
1. One topic that we haven't covered is using web mapping frameworks. We haven't