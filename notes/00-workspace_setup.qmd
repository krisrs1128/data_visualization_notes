...
2. We will be making visualizations directly within programming languages. This
...
4. To open an existing directory, go to "File > Open Folder." This will make the
...
5. We'll mainly be working with quarto files (described in detail below).  These
...
7. It's not as important, but you know that you can customize your code editor.
...
8. Literate programming is the idea that code from a programming language code
...
10. If you are using vscode, make sure to install the Quarto extension. This will
...
11. You can create a code block using three backticks and the name of the
...
12. You can modify options within each block. These won't appear in the rendered
...
13. Sometimes code prints lots of messages that can clutter the document.
...
14. It can be annoying to copy these options to every single chunk. Instead, you
...
15. To render a document, you can go to the command palette and enter `Quarto Preview` to pull up that option. You can also render from the command line with the terminal command `quarto preview your-document.qmd`.

{{<video figures/quarto_preview.mp4>}}

16. A common pitfall when using quarto is that if you take the rendered HTML
...
17. To solve this, add the option `self-contained: true` to the file header. This
will make sure that all images and file styling will be carried along with the
HTML directly.

## Agentic Coding

18. Coding agents are programs that you can call from your command line that are
...
19. These tools can be helpful in visualization, especially if you think very
...
20. In this class, I'll be asking you to be using the Pi agent for all the work
...
21. You can use pi from the terminal. In the recording below, I've used a free
...
22. At the very end of that recording, I use the `/export` command to save the
...
23. If you are done with a task and want to move on to another problem, you can
...
25. I will give you an openrouter API key that gives you access to a a few
