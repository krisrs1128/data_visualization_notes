...
4. Every plot is made from three parts,
   ...
   When we construct a data visualization, we are imprinting marks on a page in the
   same way that an artist creates a work from a blank canvas. The properties of
   these marks that we can vary are called _visual channels_. For example,
...
5. For example, below we will make this bar chart of country population in the
...
6. How many times larger (in terms of area) is the circle on the right?
...
8. One immediate implication of this is summarized as the
**Effectiveness Principle**. For any given visualization task, we should prioritize the top of the
hierarchy for the comparisons that are the most important. E.g., position and
color should be reserved for the features that are most important to compare.  This means
that even for the exact same data set, depending on the question, different
approaches might be more appropriate.  An example of this is discussed below. If
we  want to compare mileage with price across all cars. It's most appropriate to
use a scatterplot, while if we want to compare individual cars of interest, it's
easier to find this information in matched bar plots.
...
9. A related principle is the expressiveness principle. An expressive
...
10. Here is another exercise. How long does it take you to find the circle?
...
12. The _Grammar of Graphics_ is the idea that we can compose rich visualizations
...
13. We will use two software packages that implement a version of the grammar of
...
and is extremely widely used in practice.  Mosaic vgplot is a relative
newcomer, but it is well suited to interactive visualization design. We'll see
how both handle data, marks, and encodings.

**`ggplot2`**

14. Let's use the grammar of graphics to construct this plot.
...
15. Data: The first argument to `ggplot()` must be a data.frame where each row is an observation and each column is an attribute describing the observation. This is important because each mark that you see on a ggplot figure – a line, a point, a rectangle, ... – had to start out as a row within an R data.frame. The visual properties of the mark (e.g., color) are determined by the values along columns.
...
16. Marks: The words in the grammar of graphics are the geometry layers, called "geoms." We can associate each row of a data frame with points, lines, rectangles, etc., by referring to the appropriate geom. A typical plot will compose a chain of layers on top of a dataset,
...
17. Encodings: In ggplot2, we use `aes()` to implement the graphical encoding.
...
18.  The specific mapping from data values to physical mark properties are
...
19. Notice that the original visualization also had text labeling all the states
...
20. The overlay line giving the national rate is actually a bit subtle to
...
21. Finally, we can add layers that customize the plot appearance and improve the
...
22. With agentic coding, it is not so necessary to write each of these layers by
...
can read the prompts and the intermediate outputs [here](https://drive.google.com/drive/folders/1DROl3TK9XpsPWdD3FMczOZbenLZQc4NE?usp=drive_link). One point of
...
the code) and was a little bit lighter (instead of "gray50" I set it to
"#d4d4d4").
...
23. A similar grammar of graphics is implemented by Mosaic vgplot. One nuance is
...
24. We can read the analogy to ggplot2 even from a minimal scatter plot example.
...
25. As before, we can customize the scales. Here, `xScale` and `yScale` allow us to use a log scale while the color scheme allows us to switch to a colorbrewer palette.
...
26. Finally, we overlay a line giving the national rate. The syntax here differs
...
28. We'll illustrate marks with the gapminder dataset, which describes the
...
29. **Bar Marks** associate a continuous field with a categorical one. They also support alpha, color (border of the bar), fill (inside of the bar), and linetype (e.g., solid vs dashed borders) visual channels.
...
30. A few other points that I think are worth improving
...
31. This seems like a lot of work for just a lowly bar plot! But I think it's
...
32. In the plot above, each bar is anchored at 0. Instead, we could have each
...
33. **Line marks** are useful for comparing changes. Our eyes naturally focus on
...
34. **Area marks** have a flavor of both bar and line marks. The encodings are
...
35. All those examples were made with ggplot2. Mosaic doesn't support segment or
...
36. Here is the analogous lineplot. The `z` encoding groups each country's
