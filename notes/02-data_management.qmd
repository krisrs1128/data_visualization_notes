...
26. In R, we can use the mutate command to derive new variables. For example,
...
27. Sometimes, it's interesting to visualize very large datasets. Of course,
...
28. To illustrate, below we've created a database using the NYC taxi data. This
...
29. As a first step let's add a new table (which we call `taxi`) to our existing
...
30. Now we can look at the first ten rows without having to read in the entire
dataset.
...
31. To summarize, these are the main functions that we need to know for tidying
