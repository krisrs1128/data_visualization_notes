...
Readings: [1](https://clauswilke.com/dataviz/multi-panel-figures.html), [2](https://www.edwardtufte.com/notebook/sparkline-theory-and-practice-edward-tufte/) (optional), [3](https://medium.com/@hint_fm/design-and-redesign-4ab77206cf9) (optional)
...
2. There are several ways around this, though. One of the most effective is to
...
3. Our eyes are good at making sense of many small plots, as long as there is
...
4. Some important considerations are,
...
5. Here's a historical example, by Francis Walker for the 1870 US Census.  Note
...
6. Keep these kinds of plots in mind as a potential alternative to
...
7. Ridge plots are a special case of small multiples that are particularly
...
8. Why not plot the raw data? We do this using a `geom_point` below. The result
...
9. Another special case are sparklines. These are small, simplified line plots
...
10. Besides direct integration into text, it's common to see sparklines in merged
...
![](figures/0001OS-15741.gif){width="200%"}

### Multiform Plots

11. Small multiples are useful whenever we want different rows of the data to
...
12. The main advantage of multiform plots is that individual panels can be
...
13. There are a few considerations that can substantially improve the quality of
a multiform plot,
...
14. The figures below are multiform plots of a dataset of athlete physiology. They
...
15. Effective annotation can be used to refer to different subpanels of the data
...
16. Small multiples (and especially sparklines) are most legible when they trim
...
17. A startling example is this redesign of a visualization related to the Space
...
18. Nonetheless, increasing information density (and removing irrelevant marks)
...
19. There are two ggplot2 layers for making small multiples, which the package
...
20. If we have many very many panels and don't want them all in one row, we can
...
21. By default, the panels are sorted alphabetically. But to increase information
...
22. The ggridges package can be used to create ridge plots. If we want to
...
23. Sometimes we want a ridge plot and already have the height values for the
...
24. `patchwork` can be used to merge ggplot2 components into multiform plots. It
...
25. We can use `+` and `/` like algebraic symbols to compose more complex
...
26. Let's first read in the penguins dataset. We use a locally saved,
preprocessed copy so that missing values do not cause numeric columns to be
read as strings.
...
vg = {
  const mod = await import("https://cdn.jsdelivr.net/npm/@uwdata/vgplot@0.19.0/+esm");
...
penguins_db = {
  await vg.coordinator().exec([
    vg.loadCSV("penguins", path("penguins.csv"), { header: true })
  ]);
  return true;
}
```

27. `vgplot` doesn't support faceting directly, but you can compose component
...
- Compare and contrast faceting with multiform visualizations.
- Critique and suggest improvements to a visualization from the information density perspective.
- Describe best practices for ensuring readability of multiform plots.
- Given examples of ggplot2 faceting or mosaic concatenation code, describe what each element means and what the output would look like.
- Given examples of patchwork code, describe what the syntax means and what the output would look like.