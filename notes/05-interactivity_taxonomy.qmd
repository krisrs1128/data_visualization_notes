...
12. A similar options works for hover events. In the sketch below, we update the
...
13. For view coordination, we can use a selection to filter the data shown in
...
14. Let's use these patterns to build a richer example, one step at a time. Our
...
15. Before we construct this plot, let's create the coordinator. We refer to a
...
16. Before adding interactivity, let's create a static version. This forces us to
...
17. We now add a brushing **selection** to the scatterplot, without attempting to
...
18. Next let's **coordinate** the two plots, so that the barplot reflects the
...
19. Finally, let's support **selection** through clicks on the barplot and the
...
20. This matches the interactive figure at the top of the section — the only
...
21. The example above looked simple when we only looked at the plot, but the
...
22. Before giving a more abstract definition, let's see how it works in our
example above.

23. This view immediately suggests some alternatives,
...
24. More abstractly, a reactive graph summarizes how user inputs propagate
through the visualization. Nodes mean {} and edges are drawn between two nodes
whenever {}.

25. Here's toy example that illustrates why thinking of the reactive graph is
...
26. When we draw the reactive graph for this visualization, we can see a problem:
...
27. Therefore, we update the graph to trim away the redundant edges.
...
28. Whenever building an interactive visualization, I find it helpful to work
through these steps,
