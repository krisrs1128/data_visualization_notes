...
2.  Networks and trees are types of *graphs*, which are defined by (a) a set $V$
...
5. What visual comparisons do networks and trees facilitate? Viewed narrowly,
...
6. More broadly, we often visualize graphs with the goal of developing a more
...
10. What distinguishes a good from a bad graph visualization? As in all data
...
11. For example, if a visualization has too many edge crossings, then we will
...
12. In interactive graphs, we should aim to minimize the amount of change in the
...
13. One of the best R packages for graph manipulation is called `tidygraph`. The
...
14. For example, `G` below shows the friendship connections between 70 students
in a high school over two years, derived from a survey conducted in 1957 and
15. The `Node Data` component gives features for each student (in this case,
...
16. The beauty of this data structure is that we can define the analogs of the
...
17. What if we want to mutate the edges instead? We have to tell tidygraph to
...
18. There are many other verbs that have been defined for tidygraph objects. For
...
19. Similarly, we can filter nodes or edges based on their attributes.
...
20. It's possible to perform simple graph algorithms using these verbs. For
...
21. We can even map over nodes to compute topological queries. For example, the
...
22. A node-link diagram is a visual encoding strategy for network data, where
...
23. Attributes of nodes and edges can be encoded using size (node radius or edge
...
24. The node-link representation is especially effective for the task of
...
25. In node-link diagrams, spatial position is a design choice. It does not
...
26. One common layout algorithm uses force-directed placement. The edges here are
...
27. Formally, trees are a special type of graph which have no cycles (paths
...
28. To illustrate, we'll work with the following dataset, which shows the
...
29. In ggraph, this kind of visualization is constructed using the same node-link
...
30. The key drawback of node-link diagrams is that they do not scale well to
...
31. In this situation, it is possible to use additional structure in the data to
...
32. If a large network has a modular structure, then it is possible to first lay
...
33. When no such structure is available, it's often useful to try filtering or
...
34. To summarize, node-link diagrams are very good for characterizing local
structure, but struggle with large networks.

### Adjacency Matrices

35. The adjacency matrix of an undirected graph is the matrix with a 1 in entry
...
36. The example below shows the node-link and adjacency matrix plots associated
...
37. In a directed graph, the $ij^{th}$ entry is set to 1 if node $i$ leads
...
38. Node properties can be encoded along the rows and columns of the matrix. Edge
...
39. The example below takes a protein network and colors each edge according to
...
40. The key advantage of visualization using adjacency matrices is that they can
...
41. Adjacency matrices can be as useful as node-link diagrams for the task of
...
42. The key disadvantage of adjacency matrix visualization is that it's
...
43. These visualizations also tend to be less intuitive for audiences that
haven't viewed them before.

**Ordering**

44. Unlike a node-link diagram, an adjacency matrix requires us to associate each
...
45. Since ordering matters this much, it is worth choosing it deliberately rather
...
46. If nodes can be conceptually organized into a hierarchy, then it's possible
...
47. The most important representation of this kind is the treemap. In the
...
48. This is a particularly useful visualization when it's important to visualize
...
49. The main difficulty with using enclosure is that it becomes more difficult to
...
50. Hierarchy most obviously occurs in trees, but it can also be present in
...
51. An intermediate between the node-link and the treemap approach is to use an
...
52. Return to the problem of edge crossings. When many edges travel through a few
...
53. Bundled connections can be visualized using the `geom_conn_bundle` geometry
